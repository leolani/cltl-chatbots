...
import jsonpickle
import requests
from emissor.representation.annotation import NER, AnnotationType, Token
from emissor.representation.container import Index
from emissor.representation.entity import Emotion
from emissor.representation.scenario import Annotation, Mention, TextSignal
...
def add_ner_annotation_with_spacy(signal: TextSignal, nlp):
    ...
    annotate_tokens(
        signal, entity_list, segments, AnnotationType.NER.name, processor_name
    )
...
def add_np_annotation_with_spacy(signal: TextSignal, nlp, SPEAKER: str, HEARER: str):
    ...
    if speaker_tokens:
        ...
        annotate_tokens(
            signal, speaker_mentions, segments, AnnotationType.LINK.name, processor_name
        )
    ...
    if hearer_tokens:
        ...
        annotate_tokens(
            signal, hearer_mentions, segments, AnnotationType.LINK.name, processor_name
        )

    return subjects_and_objects


def get_subj_amod_triples_with_spacy(
    signal: TextSignal, nlp, SPEAKER: str, HEARER: str
):
        ...
        if token.dep_ in rels:
            ...
            if token.dep_ == "dobj" or token.dep == "xcomp":
                if token.text.lower() == "i":
                    ...
                    token.pos_ == "PROPN" or token.pos_ == "NOUN" or token.pos_ == "ADJ"
                ):
                    predicates[head_id]["tail"] = token.lemma_
                    subject_tokens.append(token)
                    subject_mentions.append(SPEAKER)
...
def get_subj_attr_triples_with_spacy(
    signal: TextSignal, nlp, SPEAKER: str, HEARER: str
):
...
# @TODO
def get_subj_prep_pobj_triples_with_spacy(
    signal: TextSignal, nlp, SPEAKER: str, HEARER: str
):
    ...
    for token in doc:
                ...
                for token_dep in doc:
                    if token_dep.dep_ == "pobj" and token_dep.head.i == token.i:
                        ...
                        if token_dep.text.lower() == "i":
                            ...
                            token_dep.pos_ == "PROPN"
                            or token_dep.pos_ == "NOUN"
                            or token_dep.pos_ == "ADJ"
                        ):
        ...
        for pred_token, pred_info in predicates.items():
            property_string = (
                doc[pred_token].lemma_ + "_" + pred_info.get("prep", str(None))
            )
            triple = (
                property_string,
                pred_info.get("head", None),
