...
def absolute_path(
    ...
    abs_path = os.path.abspath(
        os.path.join(scenario_storage.base_path, scenario_id, modality.name.lower())
    )
...
def create_image_signal(
    ...
    return ImageSignal.for_scenario(
        scenario.id, timestamp, timestamp, file_path, bounds, []
    )
...
def create_text_signal_with_speaker_annotation(
    scenario: Scenario, utterance: str, speaker: str, timestamp: int = None
):
    timestamp = int(time.time() * 1e3) if timestamp is None else timestamp
    textSignal = TextSignal.for_scenario(
        scenario.id, timestamp, timestamp, [], utterance, []
    )
