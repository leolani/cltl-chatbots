...
from emissor.representation.scenario import Scenario, TextSignal

src_path = os.path.abspath(os.path.join("../.."))
if src_path not in sys.path:
    sys.path.append(src_path)

from ..util import capsule_util as c_util
from ..util import text_util as t_util
...
def post_a_triple_and_verbalise_throughts(
    triple, replier: LenkaReplier, my_brain: LongTermMemory
):
    reply = None
    response = None
    capsule = c_util.triple_to_capsule(triple, UtteranceType.STATEMENT)
    # print(capsule)
    try:
        response = my_brain.update(
            capsule
        )  # ADDITIONAL PARAMTERS reason_types=True, create_label=False
        response_json = brain_response_to_json(response)
        reply = replier.reply_to_statement(
            response_json
        )  # ADDITIONAL PARAMETERS proactive=True, persist=True
    except:
        print("Error:", response)
    if reply is None:
...
def post_a_query_and_verbalise_answer(
    triple, replier: LenkaReplier, my_brain: LongTermMemory
):
    reply = None
    response = None
    ...
    if not chat.last_utterance.triples:
        ...
        for extracted_triple in chat.last_utterance.triples:
            ...
            capsule = c_util.scenario_utterance_to_capsule(
                scenario, place_id, location, textSignal, human_id, extracted_triple
            )
...
def process_statement_and_reply(
    scenario: Scenario,
    place_id: str,
    location: str,
    human_id: str,
...
def process_text_spacy_and_reply(
    scenario: Scenario,
    place_id: str,
    location: str,
    speaker: str,
    ...
    subject_objects = t_util.add_np_annotation_with_spacy(
        textSignal, nlp, speaker, hearer
    )
    if print_details:
    ...
    if len(entities) > 0:
        for entity in entities:
            capsule = c_util.scenario_text_mention_to_capsule(
                scenario,
                place_id,
                location,
                textSignal,
...
def process_triple_spacy_and_reply(
    scenario: Scenario,
    place_id: str,
    location: str,
    speaker: str,
    ...
    triples.extend(
        t_util.get_subj_obj_triples_with_spacy(textSignal, nlp, speaker, hearer)
    )
    if print_details:
